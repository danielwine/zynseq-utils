...
class zynseq(zynthian_engine):
	...
	def __init__(self, path=None, state_manager=None):
		self.state_manager = state_manager
		self.changing_bank = False
		path = dirname(realpath(__file__)) if path is None else path
		try:
			self.libseq = ctypes.cdll.LoadLibrary(path + "/build/libzynseq.so")
	...
	def select_bank(self, bank=None, force=False):
		...
		if self.libseq.getSequencesInBank(bank) == 0:
			print('building bank structure ', bank)
